...
def train(train_loader, model, criterion, optimizer, epoch, device, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        # if i >= warmup_batches + timing_batches:
        #     break
        ...
        if i % args.print_freq == 0:
            tag = "(warmup)" if i < warmup_batches else ""
            print(f"[Batch {i+1:03d}] {tag} Time: {batch_duration:.4f}s")

    print("\n===== TIMING SUMMARY (after warm-up) =====")
    # print(f"Batches timed: {timing_batches}")
    print(f"Total batches timed (after {warmup_batches} warmup batches): {len(train_loader) - warmup_batches}")
    print(f"Total time: {total_time:.4f} seconds")
    print(
        f"Average batch time: {total_time / max(len(train_loader) - warmup_batches,1):.4f} seconds"
    )
